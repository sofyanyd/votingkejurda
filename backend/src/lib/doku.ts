...
/**
 * Request DOKU e-Wallet / E-Money Payment Page URL (supports Rp 2.000, ShopeePay, OVO, LinkAja, DOKU Wallet, etc.)
 */
export const requestDokuEwallet = async (params: {
  invoiceId: string;
  amount: number;
  notifyUrl?: string;
}): Promise<{ paymentUrl: string; dokuReference: string; expiresAt: Date }> => {
  ...
  const requestId = `REQ-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
  const timestamp = new Date().toISOString();
  const targetPath = "/dokuwallet-emoney/v1/payment";

  const notifyUrl = params.notifyUrl || "https://votingkejurda-production.up.railway.app/api/payment/doku/webhook";
  ...
  console.log(`[DOKU EWALLET] Requesting payment URL for invoice: ${params.invoiceId}, amount: ${params.amount}`);
  ...
  const responseText = await response.text();
  console.log(`[DOKU EWALLET] Response status: ${response.status}`);
  console.log(`[DOKU EWALLET] Response body: ${responseText}`);

  if (!response.ok) {
    throw new Error(`DOKU e-Wallet API error (${response.status}): ${responseText}`);
  }
  ...
  try {
    ...
    throw new Error(`DOKU e-Wallet API returned invalid JSON: ${responseText}`);
  }

  const paymentUrl = data.doku_wallet_payment_page?.url || data.payment_url || data.url;

  if (!paymentUrl) {
    throw new Error(`DOKU e-Wallet API tidak mengembalikan payment URL. Response: ${responseText}`);
  }

  return {
    paymentUrl,
    dokuReference: data.order?.invoice_number || params.invoiceId,
    expiresAt: new Date(Date.now() + 60 * 60 * 1000)
  };
};
